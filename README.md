...
一个面向 Panasonic Lumix GF10 的本地网页照片浏览器：连接相机 Wi-Fi 后，可以读取缩略图、打开预览、多选照片，并将 JPG 或 RAW 原图逐张下载为图片文件。
...
下载栏默认选择 RAW。GF10 的目录服务会把 RAW 条目伪装成 `.JPG`，程序会在下载时将同编号文件改为 `.RW2` 请求；如果该照片没有 RAW，则自动回退到 `.JPG`。
